...
inject(function ($) {
    ...
    (function () {
        register("t", { link: tlink,
                        ...
                        displayName: function (canonicalName, match) { 
                            var verse = match[3].length ? ":" + match[3] : ''; 
                            return canonicalName + " " + match[2] + verse;
                        }
                      });
        register("g", { link: glink, 
                        ...
                        displayName: function (canonicalName, match) { return canonicalName + " " + match[2] + match[3].toLowerCase(); }
                        });
        register("mt", { link: mtlink,
                        ...
                        displayName: function (canonicalName, match, isUntouched) { 
                            var name = "Rambam, " + (isUntouched ? "Hilchot " : "") + canonicalName,
                                chapter = match[2],
                                law = match[3].length ? ":" + match[3] : "";
		...
		t.on('focusout', function () { //when you click away, I pounce!
		    if (r){
                ...
                while(true) {
                    ...
                    var replacementText = link(registrations[match[2]], match[3], match[4]);
                    if(replacementText) {
	...
	function glink (mes, match, flags) {
	...
	function tlink (book, match, flags) {
	...
	function mtlink (topic, match, flags) {
